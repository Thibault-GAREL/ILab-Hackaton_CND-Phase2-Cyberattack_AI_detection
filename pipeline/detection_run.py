...
def run_detection_chain(
    ...
    *,
    ...
    """
    batch_df : logs bruts du poll (pour detection_time_seconds ; 0 si bonus désactivé).
    """
    _t0 = pd.Timestamp.now(tz="UTC")
    ...
    if attacks:
        if BEDROCK_ENABLED:
            ...
            print(
                ...
                "sortie des détecteurs et garde-fous statiques uniquement.",
                file=sys.stderr,
            )
        apply_ds1_canonical_windows(attacks)
        apply_ds1_ioc_canonicalization(attacks)

    apply_detection_latency_seconds(attacks, batch_df, pipeline_start_time=_t0)
    attach_remediation_plans(attacks)

    _REMEDIATION_SUMMARY_EN: dict[str, str] = {
        "credential_stuffing": "Block attacker IPs on AWS WAF + Reset compromised credentials + Remove web shell",
        "ssh_brute_force": "Block IPs on Security Group + Restrict SSH to bastion + Audit authorized_keys",
        "sql_injection": "Enable WAF SQLi managed rules + Restrict DB security group + Rate limit sensitive URIs",
        "directory_traversal": "Block path traversal patterns on WAF + Review S3 public access + Tighten IAM app role",
        "ssrf": "Block internal IPs in WAF rules + Enforce IMDSv2 + Audit egress security groups",
    }

    for attack in attacks:
        cid = attack.get("challenge_id", "")
        det = attack.get("detection")
        if isinstance(det, dict):
            summary = _REMEDIATION_SUMMARY_EN.get(cid, "")
            if not summary:
                plan = attack.get("remediation", {})
                actions = plan.get("actions", [])
                titles = [a.get("title", "") for a in actions[:3]]
                summary = " + ".join(t for t in titles if t)
            if summary:
                det.setdefault("indicators", {})["remediation_proposed"] = summary
