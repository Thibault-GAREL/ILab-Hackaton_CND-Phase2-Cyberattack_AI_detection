...
def detect_ssh_brute_force(
    ...
    for campaign_keys in campaigns:
        ...
        # Fallback 3 : username le plus ciblé parmi les échecs SSH (hors comptes génériques)
        if not victim_accounts:
            top_users = merged["username"].dropna().value_counts()
            generic = {"root", "admin", "test", "guest", "oracle", "postgres",
                        "centos", "deploy", "ubuntu", "nobody"}
            for user, _ in top_users.items():
                if str(user).lower() not in generic:
                    victim_accounts = [str(user)]
                    break
